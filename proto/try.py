...
def compute(sigma, rho):
    ...
    vs = np.random.multivariate_normal(means, covs, 1000000)
...
def fun(sigma, rho):
    ...
    result = (5./4.) * rho * sigma * np.exp(.25 * sigma * sigma)
    result = (rho / sigma) * (sigma**4. + sigma**2.) * np.exp(.5 * sigma*sigma)
    result = rho * np.exp((1./8.)*sigma*sigma) * (.25*sigma*sigma*sigma + sigma)
