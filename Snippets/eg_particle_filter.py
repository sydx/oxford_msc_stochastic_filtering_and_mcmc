...
h = lambda t, X : 5.0 * X.copy()
variates = np.random.normal(loc=0.0, scale=1.0, size=(2, generationIntervalCount + 1))

# TODO: Apply h to columns:
print '(T/generationIntervalCount)', (T/generationIntervalCount)
Y = np.cumsum(X * (T/generationIntervalCount), axis=1) + np.cumsum(variates * np.sqrt(T/generationIntervalCount), axis=1)
...
def observer(t):
    ...
    print 't/(T/generationIntervalCount)', int(t/(T/generationIntervalCount))
    return Y[:,[int(t/(T/generationIntervalCount))]]
...
#particleHistoryTimes = pf.getParticleHistoryTimes()
#particleHistoryValues = pf.getParticleHistoryValues(0)

#pylab.plot(particleHistoryTimes, particleHistoryValues)
